...
export default class DraftList extends Vue {
    ...
    this.$auth
      ...
      .then((token) => {
        ...
        api(aspida())
          ...
          .then((res) => {
            this.draft_list.splice(0, this.draft_list.length, ...res);
          });
      })
      .catch(() => {
        this.forbidden = true;
  ...
  accept_draft(draft_id: number) {
    this.$auth.attempt_get_JWT("blogAdmin").then((token) => {
      api(aspida())
        ...
        .then((res) => {
          this.draft_list.splice(
            0,
            this.draft_list.length,
            ...this.draft_list.map((draft) => {
  ...
  get filteredDrafts(): Draft[] {
    switch (this.current_tab_number) {
      ...
      case 4:
        return this.draft_list.filter((draft) => draft.deleted);
      default:
        return this.draft_list.filter((draft) => !draft.deleted);
    }
  }
